...
let sheep;
const sheepLoaded = new Promise((resolve) => {
  objLoader.load('assets/sheep.obj', (sheepGeometry) => {
    sheepGeometry = sheepGeometry.children[0].geometry;
    sheepGeometry.computeVertexNormals();

    sheep = new THREE.BufferGeometry().copy(sheepGeometry);
    sheep.scale(0.7, 0.7, 0.7);
    sheep.translate(0.2, 0., 0.1);
...
let human;
const humanLoaded = new Promise((resolve) => {
  objLoader.load('assets/human.obj', (humanGeometry) => {
    humanGeometry = humanGeometry.children[0].geometry;
    human = humanGeometry;
    human.scale(0.009, 0.009, 0.009);
    human.translate(-0.5, 0.5, 0.5);
    resolve();
  });
});
...
class Caustics {
  ...
  constructor() {    this.target = new THREE.WebGLRenderTarget(waterSize * 3., waterSize * 3., {type: THREE.FloatType});
...
Promise.all(loaded).then(() => {
  const envGeometries = [floorGeometry, hand, sheep, human];
